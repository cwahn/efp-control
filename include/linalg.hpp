...
namespace efp {
    ...
    using PolyFromRoots_t = Matrix<
        ...
        A::RowsAtCompileTime == Dynamic ? Dynamic : A::RowsAtCompileTime + 1,
        1>;

    // ! Partial function. Wrong shape will abort the function.
    template <typename A>
    auto poly_from_roots(const Mat<A>& roots) -> PolyFromRoots_t<A> {
        if (A::ColsAtCompileTime != 1 && roots.cols() != 1) {
            ...
            const auto add_root = [&](int i, auto root) {
                const auto diff =
                    from_function(i + 2, [&](int i) { return i == 0 ? 0. : root * poly[i - 1]; });

                for_each_with_index([&](int i, auto x) { poly[i] -= x; }, diff);
            };
    ...
    auto _tf_num_from_ss_nm(
        ...
        int n,
        int m) {
        const auto no_feed_through = characteristic_poly((am - bm.col(n) * cm.row(m)).matrix().eval());
    ...
    auto tf_from_ss_nm(
        ...
        const auto num = _tf_num_from_ss_nm(den, am, bm, cm, dm, n, m);
    ...
    auto tf_from_ss(const Mat<A>& am, const Mat<B>& bm, const Mat<C>& cm, const Mat<D>& dm) {
        Matrix<
            std::tuple<
                ...
                decltype(_tf_num_from_ss_nm(characteristic_poly(am), am, bm, cm, dm, 0, 0))>,
            C::RowsAtCompileTime,
            B::ColsAtCompileTime>
            tfm;
        ...
        const auto fill_tfm = [&](int i, int j) {
            const auto num = _tf_num_from_ss_nm(den, am, bm, cm, dm, i, j);
